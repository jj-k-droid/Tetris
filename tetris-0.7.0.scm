...
(define update-game
  (lambda ()
    ...
    ;           (if (and (= height 360) (= time-for-respawn 0))
    ;               (spawn-shape!)
    ;               void)
    ;           #t))
  ...
  (lambda (key)
    (cond
      ...
      [(or (equal? key "ArrowRight") (equal? key "ArrowLeft") (equal? key "ArrowDown"))
           (move key canv)]
      [(equal? key "f") (begin
                          (block-freeze! tetris-grid (get-shape-grid) (deref x-position) (deref y-position))
                          (spawn-shape!))]
      [else void])))
  ...
  (animate-with
    (lambda (time)     
        (let* ([game-time (round (- (round time) (deref state-change)))]
             [move-down? (= 0 (remainder (quotient game-time 5) 200))])
            (begin
              (if move-down?
                (move "ArrowDown" canv)
                void)
